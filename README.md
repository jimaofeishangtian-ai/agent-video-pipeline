# Creator Auto Edit / Agent Video Pipeline

一套面向自媒体视频的自动剪辑 Skill。当前生产入口是 `creator-auto-edit`：它可以从真人口播、素材、文案、PPT/PPTX、PDF 或混合输入开始，自动选择剪辑与动效路线，并把整条视频统一放到 Remotion Studio 主时间轴审片。
...
素材 / 口播 / 文案 / PPT
        ↓
creator-auto-edit 统一理解、提问与语义规划
        ↓
按内容自动选择能力
├─ 真人口播整理、转录、字幕、B-roll
...
将完整的 `skill/creator-auto-edit` 文件夹复制到宿主的自定义 Skills 根目录，不能只复制 `SKILL.md`。

常见目录：

```text
Codex:      ~/.codex/skills/creator-auto-edit
WorkBuddy:  ~/.workbuddy/skills/creator-auto-edit
CodeBuddy:  ~/.codebuddy/skills/creator-auto-edit
```

也可以把下面这段话连同仓库链接交给 Agent：

```text
请从这个仓库安装并验证 creator-auto-edit Skill：
https://github.com/jimaofeishangtian-ai/agent-video-pipeline

完整复制 skill/creator-auto-edit 文件夹到当前宿主实际使用的自定义 Skills 根目录，
...
```text
使用 $creator-auto-edit，根据我的素材自动选择合适流程，先给我一次合并式创意选项，
